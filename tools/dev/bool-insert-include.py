...
def assert_text(path: Path, expected: str) -> None:
    ...
    if actual != expected:
        raise AssertionError(f"{path} contents differ\nexpected:\n{expected!r}\nactual:\n{actual!r}")


def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="insert-include-") as td:
        root = Path(td)
        ...
        if run(args) != 0:
            return 1
        assert_text(sample, "#include <stdbool.h>\n#include <stdio.h>\nint main(void);\n")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("line", nargs="?", type=int, help="1-based line number to insert before")
    parser.add_argument("line_text", nargs="?", help="line text to insert")
    parser.add_argument("--blank-before", action="store_true", help="insert a blank line before the inserted line")
    parser.add_argument("--blank-after", action="store_true", help="insert a blank line after the inserted line")
    parser.add_argument("--if-missing", action="store_true", help="do nothing if the exact line already exists")
    parser.add_argument("--write", action="store_true", help="edit the file; default is dry-run")
    parser.add_argument("--diff", action="store_true", help="print a unified diff")
    parser.add_argument("--self-test", action="store_true", help="run temporary fixture tests")
    args = parser.parse_args(argv)

    if args.self_test:
        return args
    missing = [name for name in ("file", "line", "line_text") if getattr(args, name) is None]
    if missing:
