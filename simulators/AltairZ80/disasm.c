...
/*
 * Append formatted text to the disassembly buffer and return the new
 * logical output offset, clamped to the available destination.
 */
static int PRINTF_FMT(4, 5) disasm_appendf(char *output, size_t output_size,
                                           int offset, const char *fmt, ...)
{
    ...
    if (output_size == 0 || (size_t)offset >= output_size)
        return offset;

    remaining = output_size - (size_t)offset;
    va_start(args, fmt);
    written = vsnprintf(output + offset, remaining, fmt, args);
    va_end(args);
...
/*
 * Append one character to the disassembly buffer and return the new
 * logical output offset, clamped to the available destination.
 */
static int disasm_append_char(char *output, size_t output_size, int offset,
                              char c)
{
    if (output_size == 0 || (size_t)offset >= output_size - 1)
        return offset;
...
/*
 * Preserve the legacy display of an unused segment override by
 * prepending it to the completed instruction string.
 */
static void disasm_prepend_unused_segment(char *output, size_t output_size,
                                          const char *segover)
{
    size_t used;
    size_t move_len;
    ...
    if (output_size == 0)
        return;
    if (output_size <= 3) {
        snprintf(output, output_size, "%s", segover);
        return;
    }

    used = strnlen(output, output_size);
    if (used == output_size)
        used = output_size - 1;

    move_len = used + 1;
    if (move_len > output_size - 3)
        move_len = output_size - 3;

    memmove(output + 3, output, move_len);
    output[0] = segover[0];
    output[1] = segover[1];
    output[2] = ' ';
            ...
            long offset)
{
    ...
    if (!best_p) {          /* no instruction was matched */
        snprintf(output, output_size, "db 0%02xh", data[0]);
        return 1;
    }
    ...
    if (lock)
        slen = disasm_appendf(output, output_size, slen, "lock ");
    for (i = 0; i < ins.nprefix; i++)
    switch (ins.prefixes[i]) {
      case P_REP:
        slen = disasm_appendf(output, output_size, slen, "rep ");
        break;
      case P_REPE:
        slen = disasm_appendf(output, output_size, slen, "repe ");
        break;
      case P_REPNE:
        slen = disasm_appendf(output, output_size, slen, "repne ");
        break;
      case P_A16:
        slen = disasm_appendf(output, output_size, slen, "a16 ");
        break;
      case P_A32:
        slen = disasm_appendf(output, output_size, slen, "a32 ");
        break;
      case P_O16:
        slen = disasm_appendf(output, output_size, slen, "o16 ");
        break;
      case P_O32:
        slen = disasm_appendf(output, output_size, slen, "o32 ");
        break;
    }

    for (i = 0; i < (int)elements(ico); i++)
    if ((*p)->opcode == ico[i]) {
        slen = disasm_appendf(output, output_size, slen, "%s%s", icn[i],
                whichcond(ins.condition));
        break;
    }
    if (i >= (int)elements(ico))
    slen = disasm_appendf(output, output_size, slen, "%s",
                          insn_names[(*p)->opcode]);
    colon = false;
    length += data - origdata;         /* fix up for prefixes */
    for (i=0; i<(*p)->operands; i++) {
    slen = disasm_append_char(output, output_size, slen,
                              (colon ? ':' : i==0 ? ' ' : ','));
        ...
        (ins.oprs[i].segment & SEG_RMREG))
    {
        ins.oprs[i].basereg = whichreg ((*p)->opd[i],
                        ins.oprs[i].basereg);
        if ( (*p)->opd[i] & TO )
        slen = disasm_appendf(output, output_size, slen, "to ");
        slen = disasm_appendf(output, output_size, slen, "%s",
                reg_names[ins.oprs[i].basereg-EXPR_REG_START]);
    } else if (!(UNITY & ~(*p)->opd[i])) {
        slen = disasm_append_char(output, output_size, slen, '1');
    } else if ( (*p)->opd[i] & IMMEDIATE ) {
        if ( (*p)->opd[i] & BITS8 ) {
        slen = disasm_appendf(output, output_size, slen, "byte ");
        ...
        slen = disasm_appendf(output, output_size, slen, "word ");
        } else if ( (*p)->opd[i] & BITS32 ) {
        slen = disasm_appendf(output, output_size, slen, "dword ");
        } else if ( (*p)->opd[i] & NEAR ) {
        slen = disasm_appendf(output, output_size, slen, "near ");
        } else if ( (*p)->opd[i] & SHORT ) {
        slen = disasm_appendf(output, output_size, slen, "short ");
        }
        slen = disasm_appendf(output, output_size, slen, "0x%lx",
                              ins.oprs[i].offset);
    } else if ( !(MEM_OFFS & ~(*p)->opd[i]) ) {
        slen = disasm_appendf(output, output_size, slen, "[%s%s%s0x%lx]",
                (segover ? segover : ""),
                (segover ? ":" : ""),
        ...
        slen = disasm_appendf(output, output_size, slen, "dword ");
        if ( (*p)->opd[i] & BITS64 )
        slen = disasm_appendf(output, output_size, slen, "qword ");
        if ( (*p)->opd[i] & BITS80 )
        slen = disasm_appendf(output, output_size, slen, "tword ");
        if ( (*p)->opd[i] & FAR )
        slen = disasm_appendf(output, output_size, slen, "far ");
        if ( (*p)->opd[i] & NEAR )
        slen = disasm_appendf(output, output_size, slen, "near ");
        slen = disasm_append_char(output, output_size, slen, '[');
        ...
        slen = disasm_appendf(output, output_size, slen, "%s",
                (ins.oprs[i].addr_size == 32 ? "dword " :
                 ins.oprs[i].addr_size == 16 ? "word " : ""));
        if (segover) {
        slen = disasm_appendf(output, output_size, slen, "%s:", segover);
        segover = NULL;
        }
        ...
        if (ins.oprs[i].scale > 1)
            slen = disasm_appendf(output, output_size, slen, "*%d",
                                  ins.oprs[i].scale);
        started = true;
        }
        ...
        slen = disasm_appendf(output, output_size, slen, "%c0x%lx", sign,
                ins.oprs[i].offset);
        ...
        slen = disasm_appendf(output, output_size, slen, "<operand%d>", i);
    }
    }
    if (segover) {             /* unused segment override */
    disasm_prepend_unused_segment(output, output_size, segover);
    }
    return length;
}
