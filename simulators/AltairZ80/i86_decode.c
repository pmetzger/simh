...
extern uint32_t GetBYTEExtended(uint32_t Addr);
extern void PutBYTEExtended(uint32_t Addr, const uint32_t Value);
extern int32_t AX_S;    /* AX register (8086)                           */
extern int32_t BX_S;    /* BX register (8086)                           */
extern int32_t CX_S;    /* CX register (8086)                           */
extern int32_t DX_S;    /* DX register (8086)                           */
extern int32_t CS_S;    /* CS register (8086)                           */
extern int32_t DS_S;    /* DS register (8086)                           */
extern int32_t ES_S;    /* ES register (8086)                           */
extern int32_t SS_S;    /* SS register (8086)                           */
extern int32_t DI_S;    /* DI register (8086)                           */
extern int32_t SI_S;    /* SI register (8086)                           */
extern int32_t BP_S;    /* BP register (8086)                           */
extern int32_t SPX_S;   /* SP register (8086)                           */
extern int32_t IP_S;    /* IP register (8086)                           */
extern int32_t FLAGS_S; /* flags register (8086)                        */
extern int32_t PCX_S;   /* PC register (8086), 20 bit                   */
...
uint16_t decode_rm00_address(PC_ENV *m, int rm)
{
    ...
    {
     case 0:
       return (int16_t)m->R_BX + (int16_t)m->R_SI;
       break;
     case 1:
       return (int16_t)m->R_BX + (int16_t)m->R_DI;
       break;
     case 2:
       m->sysmode |= SYSMODE_SEG_DS_SS;
       return (int16_t)m->R_BP + (int16_t)m->R_SI;
       break;
     case 3:
       m->sysmode |= SYSMODE_SEG_DS_SS;
       return (int16_t)m->R_BP + (int16_t)m->R_DI;
       break;
     case 4:
       return m->R_SI;
       break;
     case 5:
       ...
       break;
     case 6:
       offset = (int16_t)fetch_word_imm(m);
       return offset;
       break;
     case 7:
       return m->R_BX;
...
uint16_t decode_rm01_address(PC_ENV *m, int rm)
{
    ...
    displacement = (int8_t)fetch_byte_imm(m); /* !!!! Check this */
    switch(rm)
    {
     case 0:
       return (int16_t)m->R_BX + (int16_t)m->R_SI + displacement;
       break;
     case 1:
       return (int16_t)m->R_BX + (int16_t)m->R_DI + displacement;
       break;
     case 2:
       m->sysmode |= SYSMODE_SEG_DS_SS;
       return (int16_t)m->R_BP + (int16_t)m->R_SI + displacement;
       break;
     case 3:
       m->sysmode |= SYSMODE_SEG_DS_SS;
       return (int16_t)m->R_BP + (int16_t)m->R_DI + displacement;
       break;
     case 4:
       return (int16_t)m->R_SI + displacement;
       break;
     case 5:
       return (int16_t)m->R_DI + displacement;
       break;
     case 6:
       m->sysmode |= SYSMODE_SEG_DS_SS;
       return (int16_t)m->R_BP + displacement;
       break;
     case 7:
       return (int16_t)m->R_BX + displacement;
       break;
    }
    halt_sys(m);
...
uint16_t decode_rm10_address(PC_ENV *m, int rm)
{
    ...
    /* note comment on decode_rm00_address above */
    displacement = (int16_t)fetch_word_imm(m);
    switch(rm)
    {
     case 0:
       return (int16_t)m->R_BX + (int16_t)m->R_SI + displacement;
